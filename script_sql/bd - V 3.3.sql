...
create trigger pk_tem on tem instead of insert
as begin
	begin try
		begin transaction
			declare @ultimo_cod_tem int
			select @ultimo_cod_tem = max(cod_tem) from tem
			set @ultimo_cod_tem = isnull(@ultimo_cod_tem,0) +1

			insert into tem(
			cod_tem, cod_produto,
			cod_ordem			
			)
			select @ultimo_cod_tem,  cod_produto, cod_ordem	
			from inserted
			commit
		end try
	begin catch
...
create trigger pk_contem on contem instead of insert
as begin
	begin try
		begin transaction
			declare @ultimo_cod_contem int
			select @ultimo_cod_contem = max(cod_contem) from contem
			set @ultimo_cod_contem = isnull(@ultimo_cod_contem,0) +1

			insert into contem(
			cod_contem, cod_servico,
			cod_ordem			
			)
			select @ultimo_cod_contem,  cod_servico, cod_ordem	
			from inserted
			commit
		end try
	begin catch
